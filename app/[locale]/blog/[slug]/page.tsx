...
export default async function ArticlePage({ params: { locale, slug } }: Props) {
  ...
  const articleJsonLd = {
    ...
    image: article.image ?? `${siteUrl}/og-default.png`,
    author: {
      '@type': 'Person',
      name: article.author ?? 'Carlos Mendoza',
      description: 'Analista de casinos cripto y blockchain con 5 años de experiencia en iGaming.',
  ...
  const breadcrumbJsonLd = {
    ...
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: locale === 'es' ? 'Inicio' : 'Home', item: `${siteUrl}/${locale}` },
        ...
        <article className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-4 text-slate-500 text-sm mb-8">
            ...
            {article.updatedAt && article.updatedAt !== article.publishedAt && (
              <span>
                🔄 {locale === 'es' ? 'Actualizado' : 'Updated'}: {new Date(article.updatedAt).toLocaleDateString(
                  locale === 'es' ? 'es-ES' : 'en-US',
