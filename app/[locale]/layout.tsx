          ...
          <header className="bg-slate-900 border-b border-slate-800 sticky top-0 z-50">
            <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
              ...
              <nav className="hidden md:flex items-center gap-5 text-sm">
                {navLinks.map((link) => (
                  <a key={link.href} href={link.href} className="text-slate-300 hover:text-amber-400 transition-colors font-medium">
                    {link.label}
                  </a>
                ))}
                <a href="https://betfury.io/?r=LUCKYSIRKO007" target="_blank" rel="noopener noreferrer sponsored"
                  className="bg-amber-500 hover:bg-amber-400 text-white font-bold text-xs px-4 py-2 rounded-lg transition-colors">
                  🎰 BetFury
                </a>
                <a href={locale === 'es' ? '/en' : '/es'} className="text-slate-400 hover:text-white text-xs">
                  {locale === 'es' ? '🇬🇧 EN' : '🇪🇸 ES'}
                </a>
              </nav>
              <div className="flex md:hidden items-center gap-3 text-sm">
                ...
                <a href="https://betfury.io/?r=LUCKYSIRKO007" target="_blank" rel="noopener noreferrer sponsored"
                  className="bg-amber-500 text-white font-bold text-xs px-3 py-2 rounded-lg">
                  BetFury
                </a>
                <a href={locale === 'es' ? '/en' : '/es'} className="text-slate-400 text-xs">
                  {locale === 'es' ? 'EN' : 'ES'}
                </a>
              </div>
            </div>
              ...
              <div className="grid sm:grid-cols-3 gap-8 mb-8">
                <div>
                  ...
                  <p className="text-slate-500 text-sm">
                    {isEs ? 'Tu guía de referencia en casinos cripto, apuestas deportivas y criptomonedas.' : 'Your reference guide for crypto casinos, sports betting and cryptocurrencies.'}
                  </p>
                </div>
                <div>
                  ...
                  <ul className="space-y-2">
                    {navLinks.map((link) => (
                      <li key={link.href}>
                        <a href={link.href} className="text-slate-400 hover:text-amber-400 text-sm transition-colors">{link.label}</a>
                      </li>
                    ))}
                  </ul>
                </div>
