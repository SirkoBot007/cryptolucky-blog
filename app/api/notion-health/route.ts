...
export async function GET() {
  ...
  if (!dbId) return NextResponse.json({ ok: false, error: 'NOTION_DATABASE_ID not set' });
  ...
  try {
    ...
    return NextResponse.json({ ok: true, tokenPrefix: token.slice(0, 10) + '...', dbTitle: dbInfo.title?.[0]?.plain_text, total: q1.results.length, publicado: q2.results.length, slugs: q2.results.map((r: any) => (r as any).properties?.Slug?.rich_text?.[0]?.plain_text) });
  } catch (e: any) {
