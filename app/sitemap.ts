...
const LOCALES = ['es', 'en'];
const SITE_LAST_BUILD = new Date('2026-06-13');
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const articleRoutes: MetadataRoute.Sitemap = LOCALES.flatMap((locale) =>
    locale === 'es'
      ? slugs.map((slug) => ({ url: `${BASE_URL}/${locale}/blog/${slug}`, changeFrequency: 'monthly' as const, priority: 0.8, lastModified: new Date('2026-06-10') }))
      : []
  );
